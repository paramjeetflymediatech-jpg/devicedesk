...
export default function TasksPage() {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      if (data.success) {
        setCandidates(data.data.filter(u => u.role === 'Candidate' || u.role === 'Team Member')); // Include Team Member just in case some exist, but ideally Candidate
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const candidateIds = candidates.map(c => String(c.id).toLowerCase());
  const filteredTests = tasks.filter(t => candidateIds.includes(String(t.assignedTo).toLowerCase()));

  const totalPages = Math.ceil(filteredTests.length / pageSize) || 1;
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const paginatedTests = filteredTests.slice((safeCurrentPage - 1) * pageSize, safeCurrentPage * pageSize);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div>
            ...
            <h1 style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
              <FiCheckSquare style={{ color: 'var(--accent-cyan, #06b6d4)' }} /> Test & Assignment
            </h1>
            <p style={{ color: 'var(--text-secondary, #94a3b8)', fontSize: '0.875rem', marginTop: '4px' }}>
              Create and assign technical or practical tests to candidates.
            </p>
          </div>
        </div>
            ...
            <tbody>
              {paginatedTests.map((test) => (
                <tr key={test.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <td style={{ padding: '14px 16px', fontSize: '0.85rem', color: 'var(--text-secondary, #94a3b8)' }}>{test.id}</td>
                  <td style={{ padding: '14px 16px', fontWeight: 600, color: 'var(--text-primary, #f8fafc)' }}>{test.title}</td>
                  <td style={{ padding: '14px 16px', fontSize: '0.85rem' }}>{test.project_id || '—'}</td>
                  <td style={{ padding: '14px 16px', fontSize: '0.85rem' }}>{test.assignedTo || '—'}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <span className={`status-tag ${
                      test.status === 'Completed' ? 'resolved' : test.status === 'In Progress' ? 'inprogress' : 'open'
                    }`}>
                      {test.status || 'Pending'}
                    </span>
                  </td>
              ...
              {filteredTests.length === 0 && (
                <tr>
                  <td colSpan="5" style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>No tests found for candidates</td>
                </tr>
              )}
            </tbody>
          </table>
