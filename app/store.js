...
export function sendMockEmail(to, subject, body) {
  ...
  console.group("%c✉️ [DeviceDesk Email System]", "color: var(--accent-cyan); font-weight: bold; font-size: 1.1rem;");
...
export function addTask(taskData) {
  ...
  // Send assignment email notification to team member
...
export function updateTask(updatedTask, operatorName = 'Admin') {
  ...
  if (index !== -1) {
    ...
    // If status changed to Completed, trigger completion email
    ...
    // If reassigned or newly assigned, trigger assignment email
...
export function completeTask(taskId, operatorName = 'System', fileUrl = null) {
  ...
  if (index !== -1 && tasks[index].status !== 'Completed') {
    ...
    // Send completion email notification to Team Leader / Assigner
